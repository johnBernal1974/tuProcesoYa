...
class HistorialSolicitudesAcumulacionAdminPage extends StatefulWidget {
  const HistorialSolicitudesAcumulacionAdminPage({super.key});

  @override
  State<HistorialSolicitudesAcumulacionAdminPage> createState() => _HistorialSolicitudesAcumulacionAdminPageState();
}

class _HistorialSolicitudesAcumulacionAdminPageState extends State<HistorialSolicitudesAcumulacionAdminPage> {
  ...
  Widget build(BuildContext context) {
    return MainLayout(
      pageTitle: 'Solicitudes de acumulación',
      content: Center(
        child: SizedBox(
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                      return const Center(
                        child: Text(
                          "No hay solicitudes de acumulación.",
  ...
  Widget _buildEstadoCards(String role) {
    return StreamBuilder<QuerySnapshot>(
      stream: firestore.collection('acumulacion_solicitados').snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
  ...
  Future<int> _obtenerTiempoPermitido() async {
    try {
      ...
      if (configCollection.docs.isNotEmpty) {
        ...
        if (data.containsKey("tiempo_respuesta_acumulacion")) {
          final valor = (data["tiempo_respuesta_acumulacion"] as num).toInt();
          print("✅ Tiempo configurado: $valor días");
          return valor;
        } else {
          print("⚠️ Nodo 'tiempo_respuesta_acumulacion' no encontrado.");
        }
      }
    ...
    if (context.mounted) {
      Navigator.pushNamed(
        ...
        arguments: {
          ...
          'subcategoria': "Solicitud acumulación",
  ...
  String obtenerRutaSegunStatus(String status) {
    switch (status) {
      case "Enviado":
        return 'solicitudes_acumulacion_enviadas_por_correo';
      default:
        return 'atender_acumulacion_page';
    }
  }
}
