...
class HistorialSolicitudesCopiaSentenciaAdminPage extends StatefulWidget {
  const HistorialSolicitudesCopiaSentenciaAdminPage({super.key});

  @override
  State<HistorialSolicitudesCopiaSentenciaAdminPage> createState() => _HistorialSolicitudesCopiaSentenciaAdminPageState();
}

class _HistorialSolicitudesCopiaSentenciaAdminPageState extends State<HistorialSolicitudesCopiaSentenciaAdminPage> {
  ...
  Widget build(BuildContext context) {
    return MainLayout(
      pageTitle: 'Solicitudes de copia de sentencia',
      content: Center(
        child: SizedBox(
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                      return const Center(
                        child: Text(
                          "No hay solicitudes de copia de sentencia.",
  ...
  Widget _buildEstadoCards(String role) {
    return StreamBuilder<QuerySnapshot>(
      stream: firestore.collection('copiaSentencia_solicitados').snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
  ...
  Future<int> _obtenerTiempoPermitido() async {
    try {
      ...
      if (configCollection.docs.isNotEmpty) {
        ...
        if (data.containsKey("tiempo_respuesta_apelacion")) {
          final valor = (data["tiempo_respuesta_apelacion"] as num).toInt();
          print("✅ Tiempo configurado: $valor días");
          return valor;
        } else {
          print("⚠️ Nodo 'tiempo_respuesta_apelacion' no encontrado.");
        }
      }
    ...
    if (context.mounted) {
      Navigator.pushNamed(
        ...
        arguments: {
          ...
          'subcategoria': "Solicitud Apelación",
  ...
  String obtenerRutaSegunStatus(String status) {
    switch (status) {
      case "Enviado":
        return 'solicitudes_copiaSentencia_por_correo';
      case "Concedido":
        return 'solicitudes_copiaSentencia_por_correo';
      case "Negado":
        return 'solicitudes_copiaSentencia_por_correo';
      default:
        return 'atender_copiaSentencia_page';
    }
  }
}
