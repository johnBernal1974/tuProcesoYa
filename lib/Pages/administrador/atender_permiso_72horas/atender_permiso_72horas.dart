...
// Método auxiliar para evitar repetir código
  ...
  void fetchUserData() async {
    ...
    if (fetchedData != null && latestData != null && mounted) {
      ...
      if (!_isFundamentosLoaded && _fundamentosDerechoController.text.trim().isEmpty) {
      ...
      if (!_isPretencionesLoaded && _pretencionesController.text.trim().isEmpty) {
        _pretencionesController.text = generarTextoPretencionesParaPermiso72Horas();
      }

      if (!_isAnexosLoaded && _anexosController.text.trim().isEmpty) {

        // Detectar si hay hijos y documentos
        final bool tieneHijosYDocumentos =
            latestData.containsKey('hijos') &&
                latestData['hijos'] is List &&
                (latestData['hijos'] as List).isNotEmpty &&
                latestData.containsKey('documentos_hijos') &&
                latestData['documentos_hijos'] is List &&
                (latestData['documentos_hijos'] as List).isNotEmpty;

        // Convertir hijos en lista segura
        final List<Map<String, dynamic>> listaHijos =
        tieneHijosYDocumentos
      ...
      final List<Map<String, String>> listaHijos = solicitudData?.containsKey('hijos') == true
          ? List<Map<String, String>>.from(
        (solicitudData!['hijos'] as List)
            .map((hijo) => Map<String, String>.from(hijo)),
      )
          : [];


      if (!_isConsideracionesLoaded && _consideracionesController.text.trim().isEmpty) {
