...
class HistorialSolicitudesExtincionPenaAdminPage extends StatefulWidget {
  const HistorialSolicitudesExtincionPenaAdminPage({super.key});

  @override
  State<HistorialSolicitudesExtincionPenaAdminPage> createState() => _HistorialSolicitudesExtincionPenaAdminPageState();
}

class _HistorialSolicitudesExtincionPenaAdminPageState extends State<HistorialSolicitudesExtincionPenaAdminPage> {
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                      return const Center(
                        child: Text(
                          "No hay solicitudes de extinción de la pena.",
  ...
  Widget _buildEstadoCards(String role) {
    return StreamBuilder<QuerySnapshot>(
      stream: firestore.collection('extincion_pena_solicitados').snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
  ...
  Future<int> _obtenerTiempoPermitido() async {
    try {
      ...
      if (configCollection.docs.isNotEmpty) {
        ...
        if (data.containsKey("tiempo_respuesta_extincion_pena")) {
          final valor = (data["tiempo_respuesta_extincion_pena"] as num).toInt();
          print("✅ Tiempo configurado: $valor días");
          return valor;
        } else {
          print("⚠️ Nodo 'tiempo_respuesta_extincion_pena' no encontrado.");
        }
      }
    ...
    if (context.mounted) {
      Navigator.pushNamed(
        ...
        arguments: {
          ...
          'subcategoria': "Solicitud Extinción de la pena",
